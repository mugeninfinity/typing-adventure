...
import { Database, Shield, Users, Settings as SettingsIcon, Award, Bone, ClipboardList, Gift } from 'lucide-react';
...
export default function AdminPanel({ cards, onCardsChange, users, onUsersChange, achievements, onAchievementsChange, siteSettings, onSiteSettingsChange, initialCardToEdit, onEditDone }) {
    ...
    const renderManager = () => {
        switch (view) {
            ...
            case 'site':
                return <SiteSettingsManager settings={siteSettings} onSiteSettingsChange={onSiteSettingsChange} />;
            case 'mons':
                return <MonManager />;
            case 'quests':
                return <QuestManager />;
